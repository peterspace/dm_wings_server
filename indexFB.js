...
async function createFacebookEvent(eventType, eventData, res) {
  const { fbclid, sub_id_10, external_id, date, client_ip_address, client_user_agent, email, phone, first_name, last_name, city, state, zip_code, country, value, currency } = eventData;
  const ip = client_ip_address || req.headers["cf-connecting-ip"] || req.headers["x-real-ip"] || req.headers["x-forwarded-for"] || req.socket.remoteAddress || "";
